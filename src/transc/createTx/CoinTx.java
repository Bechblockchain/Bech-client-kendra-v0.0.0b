...
public class CoinTx {
	...
	public static Ctx createCoinTx(String FromCoinAddress, String ToCoinAddress, BigDecimal value) throws IOException {
		
		Ctx transaction = null;
		long timestamp = System.currentTimeMillis();
		
		if(ToCoinAddress.equals("native")) {
			ToCoinAddress = Retrie.retrieveNativeValidatorAddress();
		}
		...
		String balanceCheck = BlancChk.chkblance(ValueBytes, BytesToFro.convertStringToByteArray(Static.CTX_RANGE),FromCoinAddressBytes,ToCoinAddressBytes);
		...
		if(balanceCheck.equals("A")) {
			...
			byte[] TxSig = Hasher.generateSenderCoinSignature(senderPrivateKey, FromCoinAddress, ToCoinAddress, senderPublicKey, value, newNonce, timestamp, Static.CTX_RANGE);
			...
			byte[] range = BytesToFro.convertStringToByteArray(Static.CTX_RANGE);
